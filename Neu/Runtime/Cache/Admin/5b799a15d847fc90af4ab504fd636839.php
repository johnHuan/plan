...
<div class="body">
    <!--<section>-->
        ...
        <h3>一、实验目的与基本要求</h3>
        <h4>1. 实验目的</h4>
        <p class="text"><pre><?php echo ($result["destination"]); ?></pre></p>

    <!--</section>-->
</div>
<section class="section">
    <h4>2.基本要求</h4>
    <p class="text"><pre><?php echo ($result["BasicRequire"]); ?></pre></p>
    <h3>二、实验方式与要求</h3>
    <p><pre><?php echo ($result["StyleRequire"]); ?></pre></p>
    <h3>三、实验项目及教学安排</h3>
    ...
    <p>主要仪器设备：<pre><?php echo ($result["TInstrument"]); ?></pre></p>
    <h3>五、考核方式与成绩评定办法</h3>
    <p><pre><?php echo ($result["TEvaluate"]); ?></pre></p>
    <h3>六、实验教材及参考书</h3>
    <p><pre><?php echo ($result["TBook"]); ?></pre></p>

</section>
    <div class="menu form-group">
        <a href="/plan/index.php/Admin/Shiyan/edit?id=<?php echo ($result["id"]); ?>&ccode=<?php echo ($result["CCode"]); ?>" class="btn btn-primary pull-left">返回修改</a>
        <a href="/plan/index.php/Admin/Shiyan/confirm?id=<?php echo ($result["id"]); ?>&CCode=<?php echo ($result["CCode"]); ?>&flag=1" class="btn btn-primary pull-right">确认提交</a>
    </div>
...
//            td7.innerHTML="<input name=\"TestStyle[]\" type=\"text\" class=\"inline-input\" style=\"width: 100%\" placeholder='从上一行的单选按钮中选一个填写'>";
        ...
        $('input[name="CCode"]').on('blur', function () {
            ...
            if(this.value.trim().length == 11){
                $.ajax({
                    type: 'post',
                    url: '/plan/index.php/Admin/Shiyan/dealExcel',
                    data: this.value,
